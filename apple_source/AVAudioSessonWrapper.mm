...
// This file makes bringing support back easy... All we need is the shared session.
//
...
private:
    ...
    SessionWrapper(){
        mpSharedInstance = [AVAudioSession sharedInstance];
...
static const AVSW_ConvertLm AVSW_ConvertLmArr[] = {
    ...
    {kAudioSessionCategory_AudioProcessing, AVAudioSessionCategoryAudioProcessing},
    {kAudioSessionCategory_AmbientSound, AVAudioSessionCategoryMultiRoute},
};
    
static const int AVSW_ConvertLmArrSz = sizeof(AVSW_ConvertLmArr)/sizeof(AVSW_ConvertLmArr[0]);
...
_AudioSessionGetProperty(AudioSessionPropertyID              inID,
    ...
    switch(inID){
        ...
        case kAudioSessionProperty_AudioInputAvailable:
            *(UInt32 *)outData = [SessionWrapper::inst().sess() isInputAvailable];
            break;
        ...
        case kAudioSessionProperty_PreferredHardwareIOBufferDuration: {
            assert(sizeof(UInt32) == *ioDataSize);
            *(UInt32 *)outData = [SessionWrapper::inst().sess() IOBufferDuration];
        }
            break;
        default:
            break;
...
_AudioSessionSetProperty(AudioSessionPropertyID              inID,
    ...
    switch(inID){
        ...
        case kAudioSessionProperty_CurrentHardwareSampleRate: {
            ...
            [SessionWrapper::inst().sess() setPreferredSampleRate:*(Float64 *)inData error:nil];
        }
            break;
        case kAudioSessionProperty_PreferredHardwareIOBufferDuration: {
            assert(sizeof(UInt32) == inDataSize);
            [SessionWrapper::inst().sess() setPreferredIOBufferDuration:*(UInt32*)inData error:nil];
        }
            break;
        default:
            break;
